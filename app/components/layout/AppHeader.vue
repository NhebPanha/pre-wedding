    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 md:py-4 flex items-center justify-between">
      ...
      <NuxtLink to="/" class="flex items-center gap-2 md:gap-3 group" @click="menuOpen = false">
        <div class="relative">
          ...
          <div
            class="absolute -inset-1 bg-primary/10 rounded-full opacity-0 group-hover:opacity-100 blur transition-opacity duration-300">
          </div>
        </div>
      ...
      <nav class="hidden lg:flex items-center gap-6 xl:gap-8">
        <NuxtLink v-for="link in navLinks" :key="link.id" :to="link.to"
          ...
          {{ link.label }}
          <span class="absolute bottom-0 left-0 h-0.5 bg-primary group-hover:w-full transition-all duration-300"
            :class="{ 'w-full': isActive(link.to), 'w-0': !isActive(link.to) }"></span>
        </NuxtLink>
      </nav>
      ...
      <nav v-show="menuOpen"
        ...
        <div class="flex flex-col gap-1">
          <NuxtLink v-for="link in navLinks" :key="link.id" :to="link.to"
            ...
            active-class="text-primary bg-gray-100 dark:bg-gray-800" @click="menuOpen = false">
...
const navLinks = [
  ...
  { id: 4, label: 'ភ្ញៀវកិត្តិយស', to: '/contact', icon: 'mail' }
]
